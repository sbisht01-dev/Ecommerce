...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const productRate = getUrlParameter("rate");
  const productRating = getUrlParameter("rating");
  const productImageFullURL = getUrlParameter("image");

  // Extract the path from the full URL
  const productImagePath = productImageFullURL.replace(/^.*[\\\/]/, "");

  // Find the corresponding product object from the products array
  const selectedProduct = products.find(
    (product) =>
      product.name === productName &&
      product.price === parseFloat(productRate) &&
      product.rating.toString() === productRating
  );
  ...
  // Check if product was found and log the description
  if (selectedProduct) {
